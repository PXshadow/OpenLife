...
- The AI can be hired with in game coins // TODO set dynamic prices the more are hired / add command to see needed coins
...
- say "!YUM" to switch off and on displaying where best food is 
...
- fire holds twice as long if the room is already heated enough
...
Other changes:
- Animals can pass through: Trees, Plants, Palms, Sugarcane, Ponds, Springs

Not yet implemented:
- bigger map
- Ai territory
- maps leading to locations
- twinning
...
Heat / Temperature:
- temperature is calculated on a tile basis. More fire more heat. Walls / Floors insulate
...
Server:
- FIX backpack item disapear on fast use
- implement water map 
- HUGE: implement a bigger map, at least 4X size

Web Server
- make an own client site
- display LLM calls done / left
- create a help manual
- separate style css
- make stuff clickable / add tons more information, statistics and graphs

AI:
- FIX circular uses like getting soil or water pouch
- Make AI aware of current biome in LLM context  
- Add player and own emote to LLM context
- Add more commands for LLM like attack or exile
- Feed make reply back to llm context to respond
- AI bring water if thirsty
- AI heal if wounded (at least remove arrow) (check if player is ally, not exiled or has good karma if not an ally)
- Make AI proactively talk (if born to, if kid, if feeding, on death preparation, wounded, attackt)
- Make AI talk to each other if player is close
- Track more events that happened to AI in context for LLM
- Add no AI command to have no AI bb
- Make AI leave village if too power less (allied combat power)
- Dont join a billage with too power less (allied combat power)
- Take people along on attack  /  defence
- Allow AI to attack with stones
- add LLm model and response time in log
- add info about hire cost in LLm context and as command
- increase hire cost if over used (the more % are hired)
- add LLM calls done / left in log
- Let AI mention that its starving and cant wait (or is no ally)
- Track when the interaction was
- Only ad context from this player in the LLM
- Trace number of used LLM calls / left calls
- add in context if player is grand pa / mom / uncle
- add help to the context if help is needed
- allow to think using ? connected with help
- Add container use for pies
- implement / use super crafter *bottom up crafting instead of top down*
- use Cloud based advanced AI for setting prios and add soul / character
- add neural net or let advanced AI code AI changes

OTHER:
- give cloths (allow also for elder people if better prestige or follower)
- remove power, prestige is enough
- make AI drop horse on a fence or wait at least some sec for reconnect
...
Follow:
- AI auto change leader if in new home (might be alreay implemented)
...
- Biome changes depending on temperature
...
3. Install Git (To get the code)
sudo apt-get install git

4. Install hashlink (to excecute Haxe hl files)

# 4.1 Install build tools + all common runtime deps (for SDL/OpenGL/audio/etc. — skip some if you know you don't need them)
sudo apt update
sudo apt install -y build-essential git libpng-dev libturbojpeg-dev libvorbis-dev libopenal-dev libsdl2-dev libglu1-mesa-dev libmbedtls-dev libuv1-dev libsqlite3-dev

# 4.2 Clone & build latest (master = post-1.15 fixes as of now)
git clone https://github.com/HaxeFoundation/hashlink.git
cd hashlink
make

# 4.3 Optional but smart: install system-wide (or just use ./hl from the folder)
sudo make install

# 4.4 Verify (should print HashLink VM 1.15 or slightly newer commit hash)
hl --version
...
6. Install tmux for letting the server run after disconnect:
sudo apt-get install tmux

https://www.howtogeek.com/671422/how-to-use-tmux-on-linux-and-why-its-better-than-screen/

7. Run Open Life Server in tmux for letting the server run after disconnect
...
If you want to shut it down attach with tmux:
...
How to setup the web server (needed for https):
====
By default statistics and basic page is displayed at port 8080
If you only want to display the stats in http, use port 80 in the ServerSettings.WebServerPort
If you want to use it as proper webserver with https setup Caddy:

Prerequisites
- A VPS/cloud server (Ubuntu or Debian is easiest)
- root (or sudo) access
- A domain name (e.g. myhaxeapp.com) with A record pointing to your server’s public IP
- Your Haxe server already working on http://localhost:8080 (or any port you like)
- Ports 80 and 443 open in your firewall / security group (AWS, DigitalOcean, etc.)
...
Or use WinSCP to edit the file: /etc/caddy/Caddyfile 

Replace everything with this (change myhaxeapp.com to your domain name):

myhaxeapp.com {
    # Reverse proxy to your Haxe app
    reverse_proxy localhost:8080

    # Optional but recommended: nice headers & compression
    encode gzip zstd
    header {
