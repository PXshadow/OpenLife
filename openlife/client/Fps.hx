...
class Fps extends Object {
	...
	public function new(resolution:Int = 150, ?parent:Object) {
		...
		if (resolution > 512) resolution = 512; else if (resolution < 10) resolution = 10;
		...
		for (i in 0...packedResolution)
			graphShader.graphValues[i] = new h3d.Vector(0, 0, 0, 0);
	...
	public function alignSelf(hor:FlowAlign = Right, ver:FlowAlign = Top) {
		switch (hor) {
			// case Left: x = 5;
			case Right:
				x = getScene().width - resolution - 5;
			case Middle:
				x = Math.round((getScene().width - resolution) / 2);
			default:
				x = 5;
		}
		switch (ver) {
			// case Top: y = 5;
			case Bottom:
				y = getScene().height - graph.y - graph.height - 5;
			case Middle:
				y = Math.round((getScene().height - graph.y - graph.height) / 2);
			default:
				y = 5;
		}
	}
	...
	override private function sync(ctx:RenderContext) {
		...
		// maxStat.text = "MAX\n" + max;
		#if hl
		maxStat.text = "MEM MB\n" + Std.string(Std.int(hl.Gc.stats().currentMemory / Math.pow(1000, 2)));
		#end
		stats.text = averageFPS + " FPS\n" + Math.round(Math.round(Timer.elapsedTime * 10000) / 10) + "ms";
