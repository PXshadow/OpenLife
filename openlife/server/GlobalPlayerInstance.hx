    ...
    public function move(x:Int,y:Int,seq:Int,moves:Array<Pos>)
        {
            ...
            try
            {
                ...
                {
                    var lastPos = MoveHelper.calculateNewPos(moveHelper.newMoves, moveHelper.startingMoveTicks, this.move_speed);
                ...
                var biomeSpeed = Server.server.map.getBiomeSpeed(x + this.gx, y + this.gy);
                var isBlockingBiome = biomeSpeed < 0.1;

                if(isBlockingBiome || this.isClose(x,y,ServerSettings.MaxMovementCheatingDistanceBeforeForce) == false)
                {
                    this.forced = true;
                    this.done_moving_seqNum  = seq;
                    moveHelper.newMoves = null; // cancle all movements

                    trace('MOVE: Force!   Server ${ this.x },${ this.y }:Client ${ x },${ y }');
                }
                else
                {
                    if(this.x != x && this.y != y) trace('MOVE: NoForce! Server ${ this.x },${ this.y }:Client ${ x },${ y }');
                ...
                var newMovements = MoveHelper.calculateNewMovements(this, this.x + this.gx, this.y + this.gy, moves);
                ...
                this.move_speed = newMovements.finalSpeed;

                moveHelper.newMoves = newMovements.moves;
                moveHelper.totalMoveTime = (1/this.move_speed) * newMovements.length;
                ...
                if(this.x - moveHelper.tx> spacing || this.x - moveHelper.tx < -spacing || this.y - moveHelper.ty > spacing || this.y - moveHelper.ty < -spacing )
                {          
                    moveHelper.tx = this.x;
                    moveHelper.ty = this.y;
        
                    this.connection.sendMapChunk(this.x,this.y);
                }
                ...
                Connection.SendMoveUpdateToAllClosePlayers(this, moveHelper.totalMoveTime, newMovements.trunc, MoveHelper.moveString(moveHelper.newMoves));
