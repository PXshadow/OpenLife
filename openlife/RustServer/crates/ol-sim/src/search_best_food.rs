...
//!
//! Pure candidate scoring + conservation/seed gates; live world scan lives in
//! `lib.rs` (`search_best_food_nearby` / `search_best_food_full`).
//!
//! // Haxe: openlife.auto.AiHelper.SearchBestFood (~658–1044)
...
// ── Object parent ids (Haxe processFood specials) ────────────────────────────

/// Cooked Goose — skip the first found (keep one for knife craft).
...
/// Closed Wooden Chest / Locked Wooden Chest — Haxe `blocksRemove` patches.
...
// Haxe: ServerSettings.FoodFactorEaten* — C-SS-FULL-TABLE LiveSettings
...
// Haxe: ServerSettings.FoodFactorEatenLessThanOnePercent … MoreThanTenPercent
// C-SS-FULL-TABLE
...
// ── AI seed / reach flags (Haxe ServerAi / AiBase) ───────────────────────────
...
pub struct SearchFoodCand {
    ...
    /// When true and AI + score-distance > 4 → skip (IsDangerous).
...
// ── World food factor (pure) ────────────────────────────────────────────────

/// Haxe `WorldMap.getFoodFactor` from eaten percentage (0–100 scale) with live bands.
...
/// Haxe `ObjectData.getFoodId` — parent, or foodFromTarget parent when set.
...
/// otherwise falls back to the object itself (may be 0 → skip later).
...
/// CountCloseObjects with pile uses: matching `obj_id` → +1; matching pile → +uses.
...
/// Haxe `processFood` pure — updates counters; returns score if candidate competes.
///
...
pub fn process_food(
    ...
    // Carrot / pile: need ≥4 carrots nearby (pile uses count)
    ...
    // Global food factor (per-cand × opts fallback)
...
mod tests {
    ...
    fn process_food_uses_live_yum_bonus_band() {
        let mut counters = SearchFoodCounters::new();
        // food_value 12, count 5 → scored base 7 (not superMeh: 7 >= 6).
        // Under band 5: meh (count not < 5). Under band 7: yum → ×starving_factor.
        let c = cand(33, 12, 12, 10, 5.0, 1);
        // food_store=2 → starving_factor 4 when yum
    ...
    fn food_factor_for_id_rollup_higher_quality() {
        // base 0% → 2.5; with higher quality at 4% → total 4% → 1.5
    ...
    fn food_factor_changes_ranking() {
        ...
        common.food_factor = 0.5; // ≥10% eaten band
        ...
        assert_eq!(j, 1, "2.5× factor should beat modest distance gap");
    }
    ...
    fn torus_wrap_prefers_across_map_edge() {
        ...
        opts.map_h = 100;
        opts.wrap = true;
        // Without wrap: (99,0) is far (99²); with wrap: dx=-1 → dist 1
