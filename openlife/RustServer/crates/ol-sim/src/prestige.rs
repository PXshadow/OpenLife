...
pub use ol_identity::{
    prestige_class_name_at_index, prestige_class_wire_token, PrestigeClass, CLASS_BONI_NOBLE_SERF,
    CLASS_BONI_SAME, NOOB_NOBLE_BIRTH_CHANCE, NOOB_NOBLE_MAX_LIVES, PRESTIGE_CLASS_NAMES,
...
pub fn calculate_class_boni(self_class: PrestigeClass, other_class: PrestigeClass) -> f32 {
    ...
    let noble_serf = matches!(
        ...
        (PrestigeClass::Noble, PrestigeClass::Serf) | (PrestigeClass::Serf, PrestigeClass::Noble)
    );
    if noble_serf {
...
mod tests {
    ...
    fn living_scores_empty_and_alone() {
        ...
        assert_eq!(
            prestige_class_from_percentile(0, 1),
            PrestigeClass::Commoner
        );
    }
