...
/// Haxe `ServerSettings.YumBonus` — first eats of a food id grant bonus fill.
///
/// Live path uses [`resolve_yum_bonus`] / `*_ex` with `SimState.gameplay.yum_bonus`.
...
/// Sanitize live / override YumBonus (Haxe static ≥ 0; NaN/neg → default).
...
/// Super-meh refuse threshold: Haxe `canEatObj` / self-eat refuse when `food_store > 4`.
pub const SUPER_MEH_REFUSE_FOOD_STORE: f32 = 4.0;

/// Meh feed-other refuse: Haxe `canFeedToMeObj` when `food_store > 2`.
...
///
/// `count_eaten` is `hasEatenMap[foodId]` (0 when absent; negative = craving).
...
// Haxe: GlobalPlayerInstance.isObjMeh
pub fn is_obj_meh_ex(food_value: i32, count_eaten: f32, yum_bonus: f32) -> bool {
    !is_obj_yum_ex(food_value, count_eaten, yum_bonus)
}

/// Haxe `isObjMeh`: not yum (includes non-food as meh).
...
/// `foodValue + YumBonus - max(0,countEaten) < foodValue / 2`.
// Haxe: GlobalPlayerInstance.isObjSuperMeh
...
/// - foodValue > 0
/// - refuse superMeh when `food_store > 4`
/// - room ≥ ceil(foodValue / 4)
// Haxe: GlobalPlayerInstance.canEatObj
...
// Haxe: GlobalPlayerInstance.canFeedToMeObj
...
/// Haxe `canFeedToMeObj` (meh feed only if starving ≤ 2).
///
/// Does **not** apply the 837/yellow-fever gate — use [`can_feed_to_me_obj_ex`]
/// when the food parent id is known (SearchBestFood / feed-other).
...
pub fn can_feed_to_me_obj_ex_yum(
    ...
    // only feed 837 ⇒ Psilocybe Mushroom to others if ill
...
/// Haxe `canFeedToMeObj` with Psilocybe (837) + yellow-fever gate.
// Haxe: GlobalPlayerInstance.canFeedToMeObj
...
/// Haxe-ish starving factor from food store.
