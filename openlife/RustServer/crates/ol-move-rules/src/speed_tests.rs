...
#[test]
fn shoes_speed_factor_ex_live_override() {
    assert!((shoes_speed_factor_ex(true, false, 1.5) - 1.5).abs() < 1e-6);
    assert!((shoes_speed_factor_ex(true, true, 1.5) - 1.0).abs() < 1e-6);
    assert!((shoes_speed_factor_ex(false, false, 1.5) - 1.0).abs() < 1e-6);
    let mut v = VitalsSpeedInput::default();
    v.has_both_shoes = true;
    let mut knobs = VitalsSpeedLiveKnobs::default();
    let a = vitals_speed_product_live(&v, &knobs);
    knobs.speed_with_both_shoes = 1.5;
    let b = vitals_speed_product_live(&v, &knobs);
    assert!((b / a - 1.5 / SPEED_WITH_BOTH_SHOES).abs() < 1e-4);
}
...
/// Live MinSpeedReductionPerContainedObj changes the contained upper clamp.
// SETTINGS-LONG-TAIL
#[test]
fn contained_obj_speed_mult_ex_live_clamp() {
    let default = contained_obj_speed_mult(1.0);
    assert!((default - MIN_SPEED_REDUCTION_PER_CONTAINED).abs() < 1e-5);
    let live = contained_obj_speed_mult_ex(1.0, 0.9);
    assert!((live - 0.9).abs() < 1e-5);
    let nest = held_nest_speed_product_ex(&ContentDb::default(), Some(&NestedHelper::from_wire(50, &[9999])), 0.9);
    assert!((nest - 0.9).abs() < 1e-5);
}
...
#[test]
fn half_penalty_for_strong_halves_slowdown() {
    let p = 0.64f32;
    let strong = half_penalty_for_strong(p, true);
    assert!((strong - 0.82).abs() < 1e-5, "got {strong}");
    assert!((half_penalty_for_strong(p, false) - p).abs() < 1e-5);
    assert!((half_penalty_for_strong(1.0, true) - 1.0).abs() < 1e-5);
}

#[test]
fn speed_prestige_class_is_strong_noble_plus() {
    assert!(!SpeedPrestigeClass::NotSet.is_strong());
    assert!(!SpeedPrestigeClass::Serf.is_strong());
    assert!(!SpeedPrestigeClass::Commoner.is_strong());
    assert!(SpeedPrestigeClass::Noble.is_strong());
    assert!(SpeedPrestigeClass::King.is_strong());
    assert!(SpeedPrestigeClass::Emperor.is_strong());
    assert!(SpeedPrestigeClass::from_u8(3).is_strong());
    assert!(SpeedPrestigeClass::from_u8(4).is_strong());
    assert!(!SpeedPrestigeClass::from_u8(2).is_strong());
}

#[test]
fn ai_class_speed_factors() {
    assert!((ai_class_speed_factor(false, SpeedPrestigeClass::Serf) - 1.0).abs() < 1e-5);
    assert!((ai_class_speed_factor(true, SpeedPrestigeClass::Serf) - AI_SPEED_FACTOR_SERF).abs() < 1e-5);
    assert!(
        (ai_class_speed_factor(true, SpeedPrestigeClass::Commoner) - AI_SPEED_FACTOR_COMMONER).abs()
            < 1e-5
    );
    assert!((ai_class_speed_factor(true, SpeedPrestigeClass::Noble) - AI_SPEED_FACTOR_NOBLE).abs() < 1e-5);
}
...
fn ai_class_speed_factor_ex_live() {
    // C-SS-MORE-BATCH5
    assert!(
        (ai_class_speed_factor_ex(true, SpeedPrestigeClass::Serf, 0.7, 0.85, 1.1) - 0.7).abs() < 1e-5
    );
    assert!(
        (ai_class_speed_factor_ex(true, SpeedPrestigeClass::Commoner, 0.7, 0.85, 1.1) - 0.85).abs()
            < 1e-5
    );
    assert!(
        (ai_class_speed_factor_ex(true, SpeedPrestigeClass::Noble, 0.7, 0.85, 1.1) - 1.1).abs() < 1e-5
    );
    assert!((close_enemy_speed_factor_ex(true, 0.5) - 0.5).abs() < 1e-5);
    assert!((close_enemy_speed_factor_ex(false, 0.5) - 1.0).abs() < 1e-5);
    assert!((grave_curse_speed_factor(false) - 1.0).abs() < 1e-5);
    assert!((grave_curse_speed_factor(true) - 0.9).abs() < 1e-5);
    assert!((grave_curse_speed_factor_ex(true, 0.7) - 0.7).abs() < 1e-5);
    assert!((grave_curse_speed_factor_ex(false, 0.7) - 1.0).abs() < 1e-5);
}
