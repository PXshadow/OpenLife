...
use ol_world::{
    ...
    PASSABLE_RIVER, RIVER,
};

use crate::path_steps::{client_path_deltas_to_steps, step_len, MAX_CLIENT_PATH_STEPS};

/// Prestige class tags used only for AI move-speed factors (same discriminants as sim).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
#[repr(u8)]
pub enum SpeedPrestigeClass {
    NotSet = 0,
    Serf = 1,
    #[default]
    Commoner = 2,
...
impl SpeedPrestigeClass {
    ...
    /// Haxe `Lineage.isNobleOrMore` — Noble / King / Emperor.
    ///
    /// Live `is_strong` for contained-speed half-penalty (MoveHelper L162 TODO).
    #[inline]
    pub fn is_strong(self) -> bool {
        matches!(self, Self::Noble | Self::King | Self::Emperor)
    }
}
...
///
/// `bloody_override` — COMBAT-BLOODY PatchObjectData mult (0.75/0.85/0.6) from sim weapons.
// Haxe: ServerSettings.PatchObjectData speedMult 750/3048/749
#[inline]
pub fn held_object_speed_mult_ex(
    content: &ContentDb,
    held_id: i32,
    bloody_override: Option<f32>,
) -> f32 {
    if held_id <= 0 {
        return 1.0;
    }
    if let Some(m) = bloody_override {
        return m;
    }
    content
        .get(held_id)
...
/// Content-only held speed (no bloody override). Prefer [`held_object_speed_mult_ex`] from sim.
#[inline]
pub fn held_object_speed_mult(content: &ContentDb, held_id: i32) -> f32 {
    held_object_speed_mult_ex(content, held_id, None)
}
...
/// Haxe `calculateObjSpeedMult` — clamp each contained object mult to \[0.6, MinSpeedReduction\].
#[inline]
pub fn contained_obj_speed_mult(obj_speed_mult: f32) -> f32 {
    contained_obj_speed_mult_ex(obj_speed_mult, MIN_SPEED_REDUCTION_PER_CONTAINED)
}

/// Live-knob variant of [`contained_obj_speed_mult`].
// Haxe: MoveHelper.calculateObjSpeedMult + ServerSettings.MinSpeedReductionPerContainedObj
// SETTINGS-LONG-TAIL
#[inline]
pub fn contained_obj_speed_mult_ex(obj_speed_mult: f32, min_reduction: f32) -> f32 {
    let m = if obj_speed_mult.is_finite() {
        obj_speed_mult
    } else {
        1.0
    };
    let cap = if min_reduction.is_finite() && min_reduction > 0.0 {
        min_reduction
    } else {
        MIN_SPEED_REDUCTION_PER_CONTAINED
    };
    m.min(cap).max(CONTAINED_SPEED_FLOOR)
}
...
pub fn backpack_speed_product(content: &ContentDb, backpack: &[i32]) -> f32 {
    backpack_speed_product_ex(content, backpack, MIN_SPEED_REDUCTION_PER_CONTAINED)
}

/// Live-knob variant of [`backpack_speed_product`].
// SETTINGS-LONG-TAIL
pub fn backpack_speed_product_ex(
    content: &ContentDb,
    backpack: &[i32],
    min_reduction: f32,
) -> f32 {
    ...
    for &id in backpack {
        ...
        let sm = content
            ...
            .map(|d| d.speed_mult)
            .unwrap_or(1.0);
        p *= contained_obj_speed_mult_ex(sm, min_reduction);
    }
    p
}
...
pub fn shoes_speed_factor(has_both_shoes: bool, on_horse_or_car: bool) -> f32 {
    shoes_speed_factor_ex(has_both_shoes, on_horse_or_car, SPEED_WITH_BOTH_SHOES)
}

/// Live-knob variant of [`shoes_speed_factor`].
// Haxe: ServerSettings.SpeedWithBothShoes = 1.1
// SETTINGS-LONG-TAIL
#[inline]
pub fn shoes_speed_factor_ex(
    has_both_shoes: bool,
    on_horse_or_car: bool,
    factor: f32,
) -> f32 {
...
pub fn grave_curse_speed_factor(curse_active: bool) -> f32 {
    grave_curse_speed_factor_ex(curse_active, CLOSE_GRAVE_SPEED_MALI)
}

/// Live-knob variant of [`grave_curse_speed_factor`].
// Haxe: ServerSettings.CloseGraveSpeedMali
// SETTINGS-LONG-TAIL
#[inline]
pub fn grave_curse_speed_factor_ex(curse_active: bool, factor: f32) -> f32 {
    if !curse_active {
        return 1.0;
    }
    if factor.is_finite() && factor >= 0.0 {
...
pub fn ai_class_speed_factor(is_ai: bool, class: SpeedPrestigeClass) -> f32 {
...
pub fn ai_class_speed_factor_ex(
    ...
    match class {
        SpeedPrestigeClass::Serf => s,
        SpeedPrestigeClass::Commoner | SpeedPrestigeClass::NotSet => c,
        SpeedPrestigeClass::Noble | SpeedPrestigeClass::King | SpeedPrestigeClass::Emperor => n,
    }
}
...
pub struct VitalsSpeedInput {
    ...
    pub is_ai: bool,
    pub prestige_class: SpeedPrestigeClass,
...
impl Default for VitalsSpeedInput {
    fn default() -> Self {
        Self {
            ...
            is_ai: false,
            prestige_class: SpeedPrestigeClass::Commoner,
            is_strong: false,
...
impl Default for VitalsSpeedLiveKnobs {
    fn default() -> Self {
        Self {
            ...
            speed_with_both_shoes: SPEED_WITH_BOTH_SHOES,
            close_grave_speed_mali: CLOSE_GRAVE_SPEED_MALI,
            temperature_speed_impact: TEMPERATURE_SPEED_IMPACT,
            min_speed_reduction_per_contained_obj: MIN_SPEED_REDUCTION_PER_CONTAINED,
        }
    }
}
...
pub fn vitals_speed_product_live(v: &VitalsSpeedInput, knobs: &VitalsSpeedLiveKnobs) -> f32 {
    let shoes = shoes_speed_factor_ex(
        v.has_both_shoes,
        v.on_horse_or_car,
        knobs.speed_with_both_shoes,
    );
    ...
    let temp_impact = if knobs.temperature_speed_impact.is_finite()
        && knobs.temperature_speed_impact >= 0.0
    {
        knobs.temperature_speed_impact
    } else {
    ...
    shoes
        * hp
        * temp
        * grave_curse_speed_factor_ex(v.curse_active, knobs.close_grave_speed_mali)
        * close_enemy_speed_factor_ex(
...
pub fn apply_held_floor_speed_ex(
    ...
    apply_held_floor_speed_ex_shoes(
        base_speed,
        floor_id,
        floor_spd,
        biome_spd,
...
fn apply_held_floor_speed_ex_shoes(
    base_speed: f32,
    floor_id: i32,
    floor_spd: f32,
    biome_spd: f32,
    ...
    let shoe_walk = shoes_speed_factor_ex(has_both_shoes, on_horse, shoe_walk_factor);
...
pub fn apply_held_floor_speed_at_ex(
    ...
    apply_held_floor_speed_at_ex_bloody(
        world,
        content,
        tx,
        ty,
...
/// [`apply_held_floor_speed_at_ex`] with COMBAT-BLOODY held speed override.
pub fn apply_held_floor_speed_at_ex_bloody(
    world: &World,
    content: &ContentDb,
    tx: i32,
    ty: i32,
    ...
    let held_sm = held_object_speed_mult_ex(content, held_id, bloody_override);
    // Haxe getPackpack().containedObjects; flat backpack is legacy fallback.
    let pack = resolve_backpack_speed_product_ex(
        content,
        backpack,
        clothing_pack,
        min_contained_reduction,
    );
    apply_held_floor_speed_ex_shoes(
        base_speed,
        floor_id,
...
pub fn apply_calculate_speed_full(
    ...
    apply_calculate_speed_full_bloody(
        world,
        content,
        tx,
        ty,
...
/// [`apply_calculate_speed_full`] with COMBAT-BLOODY held override (from sim weapons).
pub fn apply_calculate_speed_full_bloody(
    world: &World,
    content: &ContentDb,
    tx: i32,
    ty: i32,
    ...
    let floor_core = apply_held_floor_speed_at_ex_bloody(
        world,
        content,
        tx,
        ty,
...
pub fn apply_calculate_speed_full_live(
    ...
    apply_calculate_speed_full_live_bloody(
        world,
        content,
        tx,
        ty,
...
/// Live-knob + COMBAT-BLOODY held override.
pub fn apply_calculate_speed_full_live_bloody(
    world: &World,
    content: &ContentDb,
    tx: i32,
    ty: i32,
    ...
    let floor_core = apply_held_floor_speed_at_ex_bloody(
        ...
        bloody_override,
        knobs.speed_with_both_shoes,
        knobs.min_speed_reduction_per_contained_obj,
    );
    let mut v = *vitals;
