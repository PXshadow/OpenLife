...
pub fn is_grave_basket_target(target_id: i32) -> bool {
    matches!(target_id, FRESH_GRAVE | GRAVE | OLD_GRAVE | BONE_PILE)
}
...
pub fn should_nest_swap_helpers(
    ...
    is_pickup_or_drop || is_horse_drop_trans(change_held, new_actor_id, held_contained_count)
}
...
pub fn empty_ground_dismount_transition(content: &ContentDb, held_id: i32) -> Option<&Transition> {
    if held_id <= 0 {
        return None;
    }
...
mod tests {
    ...
    fn horse_eat_plan_food_and_drugs() {
        ...
        assert_eq!(horse_eat_plan(&db, 770, 31, false), Some((31, false, 0)));
        ...
        assert_eq!(horse_eat_plan(&db, 778, 30, false), Some((31, true, 30)));
    }
    ...
    fn nest_swap_preserves_cargo() {
        ...
        let (new_held, new_tile) = apply_ids_after_nest_swap(nest, held, 778, 0, 12.5);
    ...
    fn hitch_nest_swap_cargo_to_hitched_cart() {
        ...
        let (new_held, new_tile) = apply_ids_after_nest_swap(tile, held, 0, 779, 5.0);
        ...
        // nested_to_complex must remain is_complex so World keeps the helper.
        let cx = nested_to_complex(&new_tile, 5.0);
        assert!(cx.is_complex(), "hitched cart with cargo must be complex");
        assert_eq!(cx.contained, vec![33, 40]);
    }

    #[test]
    fn is_horse_drop_trans_true_with_cargo_empty_hand() {
        assert!(should_nest_swap_helpers(false, true, 0, 1));
        assert!(!should_nest_swap_helpers(false, true, 0, 0));
        assert!(should_nest_swap_helpers(true, true, 778, 0));
    }
    ...
    fn unhitch_nest_swap_cargo_to_held_cart() {
        ...
        let held = NestedHelper::empty();
        let (new_held, new_tile) = apply_ids_after_nest_swap(tile, held, 778, 4154, 9.0);
        assert_eq!(new_held.id, 778);
    ...
    // Live USE hitch tests stay in ol-sim (`horse_mount_live_tests.inc.rs`).
}
