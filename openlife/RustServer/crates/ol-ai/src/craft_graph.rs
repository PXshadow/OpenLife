...
pub struct ReverseCraftGraph {
    ...
    /// Haxe `TransitionData.aiShouldIgnore` edges — craft search skips these.
    /// Populated from [`ol_content::ContentDb::ai_should_ignore`] at graph seed.
    // Haxe: TransitionData.aiShouldIgnore (C-SS-AI-IGNORE)
    ai_should_ignore: HashSet<(i32, i32)>,
}
...
impl ReverseCraftGraph {
    ...
    /// Mark edge `(actor, target)` as AI-ignored (Haxe `aiShouldIgnore`).
    // Haxe: TransitionData.aiShouldIgnore
    #[inline]
    pub fn mark_ai_should_ignore(&mut self, actor: i32, target: i32) {
        self.ai_should_ignore.insert((actor, target));
    }

    /// Bulk-load ignore edges from content patches.
    // Haxe: ServerSettings.PatchTransitions → ContentDb.ai_should_ignore
    pub fn load_ai_should_ignore_from(
        &mut self,
        edges: impl IntoIterator<Item = (i32, i32)>,
    ) {
        self.ai_should_ignore.extend(edges);
    }

    /// True when craft AI should skip transition `(actor, target)`.
    // Haxe: TransitionData.aiShouldIgnore
    #[inline]
    pub fn ai_should_ignore_edge(&self, actor: i32, target: i32) -> bool {
        self.ai_should_ignore.contains(&(actor, target))
    }

    /// Number of AI-ignore edges loaded (tests / debug).
    #[inline]
    pub fn ai_should_ignore_count(&self) -> usize {
        self.ai_should_ignore.len()
    }
    ...
    pub fn find_path_to_product(
        ...
        while let Some((need, path)) = queue.pop_front() {
            ...
            for &(actor, target) in options {
                // Skip AI-ignored edges when pathfinding for craft.
                // Haxe: DoTransitionSearch continue on aiShouldIgnore
...
mod tests {
    ...
    // Haxe: TransitionData.aiShouldIgnore skips edge in craft path (C-SS-AI-IGNORE)
    #[test]
    fn ai_should_ignore_skips_path_edge() {
        let mut g = ReverseCraftGraph::new();
        g.insert(1, 2, 3, 0);
        g.insert(4, 5, 3, 0); // alternate path to product 3
        g.mark_ai_should_ignore(1, 2);
        let have: HashSet<i32> = [1, 2, 4, 5].into_iter().collect();
        let path = g.find_path_to_product(3, &have, 4).expect("alt path");
        assert_eq!(path, vec![(4, 5)]);
        assert!(g.ai_should_ignore_edge(1, 2));
        assert!(!g.ai_should_ignore_edge(4, 5));
    }

    #[test]
    fn load_ai_should_ignore_from_content_set() {
        let mut g = ReverseCraftGraph::new();
        g.load_ai_should_ignore_from([(58, 235), (516, 82)]);
        assert_eq!(g.ai_should_ignore_count(), 2);
        assert!(g.ai_should_ignore_edge(58, 235));
    }

    // Haxe: ignored edge dropped from ingredients_for fallback seek
    #[test]
    fn ai_should_ignore_skips_seek_ingredient_fallback() {
        let mut g = ReverseCraftGraph::new();
        g.insert(1, 2, 3, 0);
        g.mark_ai_should_ignore(1, 2);
        let have = HashSet::new();
        // Only reverse edge is ignored → no seek from product ingredients.
