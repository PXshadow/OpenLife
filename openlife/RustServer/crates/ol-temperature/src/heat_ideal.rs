...
/// Haxe `ServerSettings.FoodUsePerSecond` default in HEAT `foodDrainTime`.
pub const HEAT_FOOD_USE_PER_SECOND: f32 = 0.10;
/// Haxe `TemperatureHitsDamageFactor` default in HEAT `foodDrainTime`.
pub const HEAT_TEMPERATURE_HITS_DAMAGE_FACTOR: f32 = 0.5;
/// Haxe `TemperatureExhaustionDamageFactor` default in HEAT `foodDrainTime`.
pub const HEAT_TEMPERATURE_EXHAUSTION_DAMAGE_FACTOR: f32 = 0.2;
/// Absolute heat for ×2 HEAT damage factor (Haxe `heat > 0.95`).
pub const HEAT_DAMAGE_DOUBLE_HOT: f32 = 0.95;
/// Absolute heat for ×2 HEAT damage factor (Haxe `heat < 0.05`).
pub const HEAT_DAMAGE_DOUBLE_COLD: f32 = 0.05;

/// Haxe GPI `updateTemperature` `temperatureFoodFactor` / `foodUsePerSecond` / `foodDrainTime`.
///
/// `foodUsePerSecond = FoodUsePerSecond * (heat >= 0.5 ? heat : 1 - heat)`.
/// Super-hot/cold (caller, person-color gates) add
/// `damageFactor * (HitsDamageFactor + ExhaustionDamageFactor)` to the divisor
/// (`damageFactor` is 2 when heat > 0.95 / < 0.05, else 1). Indoor bonus stays 0.
///
/// Returns `(foodUsePerSecond, foodDrainTime)` with drain-time rounded to 0.01s.
/// Does not apply biome/day/weather extras — those stay on the food-eating path.
// Haxe: GlobalPlayerInstance.updateTemperature L6530–6548
pub fn heat_food_drain_time(
    heat: f32,
    is_super_hot: bool,
    is_super_cold: bool,
    food_use_per_second_base: f32,
    hits_damage_factor: f32,
    exhaustion_damage_factor: f32,
) -> (f32, f32) {
    let heat = clamp_heat(heat);
    let temperature_food_factor = if heat >= IDEAL_HEAT {
        heat
    } else {
        1.0 - heat
    };
    let mut temperature_damage_factor = 0.0;
    if is_super_hot {
        temperature_damage_factor = if heat > HEAT_DAMAGE_DOUBLE_HOT {
            2.0
        } else {
            1.0
        };
    } else if is_super_cold {
        temperature_damage_factor = if heat < HEAT_DAMAGE_DOUBLE_COLD {
            2.0
        } else {
            1.0
        };
    }
    let hits = if hits_damage_factor.is_finite() && hits_damage_factor >= 0.0 {
        hits_damage_factor
    } else {
        HEAT_TEMPERATURE_HITS_DAMAGE_FACTOR
    };
    let exh = if exhaustion_damage_factor.is_finite() && exhaustion_damage_factor >= 0.0 {
        exhaustion_damage_factor
    } else {
        HEAT_TEMPERATURE_EXHAUSTION_DAMAGE_FACTOR
    };
    let temperature_damage_factor2 =
        temperature_damage_factor * hits + temperature_damage_factor * exh + 1.0;
    let base = if food_use_per_second_base.is_finite() && food_use_per_second_base > 0.0 {
        food_use_per_second_base
    } else {
        HEAT_FOOD_USE_PER_SECOND
    };
    let food_use_per_second = base * temperature_food_factor;
    let denom = food_use_per_second * temperature_damage_factor2;
    let food_drain_time = if denom.is_finite() && denom > 0.0 {
        ((1.0 / denom) * 100.0).round() / 100.0
    } else {
        0.0
    };
    (food_use_per_second, food_drain_time)
}
...
/// Closest-heat / held heat / clothing rValue matrix live in `map_temp_player`.
...
/// Live-knob body heat step (Haxe `TemperatureImpactPerSec*`).
// Haxe: ServerSettings.TemperatureImpactPerSec / IfGood / TemperatureInWaterFactor
// SETTINGS-LONG-TAIL
pub fn body_heat_step_ex(
    heat: f32,
    ambient: f32,
    dt: f32,
    clothing_factor: f32,
    ...
    let bad = if impact_per_sec.is_finite() && impact_per_sec > 0.0 {
        impact_per_sec
    } else {
        TEMPERATURE_IMPACT_PER_SEC
    };
    let good = if impact_per_sec_if_good.is_finite() && impact_per_sec_if_good > 0.0 {
        impact_per_sec_if_good
    } else {
        TEMPERATURE_IMPACT_PER_SEC_IF_GOOD
    };
    let time_factor = if new_temp_is_positive { good } else { bad };
    let water_mul = if in_water_factor.is_finite() && in_water_factor > 0.0 {
        in_water_factor
    } else {
        TEMPERATURE_IN_WATER_FACTOR
    };
    let water_factor = if in_water { water_mul } else { 1.0 };
...
mod tests {
    ...
    #[test]
    fn heat_food_drain_time_ideal_is_twenty() {
        // 0.10 * 0.5 = 0.05 → 1/0.05 = 20.00; not super → damageFactor2 = 1.
        let (use_ps, drain) = heat_food_drain_time(0.5, false, false, 0.10, 0.5, 0.2);
        assert!((use_ps - 0.05).abs() < 1e-6);
        assert!((drain - 20.00).abs() < 1e-6);
        let (_, coldish) = heat_food_drain_time(0.3, false, false, 0.10, 0.5, 0.2);
        // factor = 0.7 → 1/0.07 ≈ 14.29
        assert!((coldish - 14.29).abs() < 1e-6);
        let (_, warmish) = heat_food_drain_time(0.7, false, false, 0.10, 0.5, 0.2);
        assert!((warmish - 14.29).abs() < 1e-6);
    }

    #[test]
    fn heat_food_drain_time_superhot_scales_damage() {
        // Super-hot heat 0.81 (< 0.95): damageFactor=1, factor2=1.7, use=0.081
        // 1/(0.081*1.7) ≈ 7.26
        let (use_ps, drain) = heat_food_drain_time(0.81, true, false, 0.10, 0.5, 0.2);
        assert!((use_ps - 0.081).abs() < 1e-6);
        assert!((drain - 7.26).abs() < 1e-6);
        // Super-hot heat 0.96: damageFactor=2, factor2=2.4, use=0.096
        // 1/(0.096*2.4) ≈ 4.34
        let (_, drain2) = heat_food_drain_time(0.96, true, false, 0.10, 0.5, 0.2);
        assert!((drain2 - 4.34).abs() < 1e-6);
        // Super-cold heat 0.0: factor=1, damageFactor=2, same 2.4 divisor → 4.17
        let (use0, drain0) = heat_food_drain_time(0.0, false, true, 0.10, 0.5, 0.2);
        assert!((use0 - 0.10).abs() < 1e-6);
        assert!((drain0 - 4.17).abs() < 1e-6);
    }
    ...
    fn body_heat_step_good_temp_is_faster() {
        ...
        assert!(good_delta > bad_delta, "good={good_delta} bad={bad_delta}");
    }
    ...
    #[test]
    fn body_heat_step_ex_live_impact_scales_delta() {
        let slow = body_heat_step_ex(0.5, 1.0, 1.0, 1.0, false, 0.01, 0.02, 1.5);
        let fast = body_heat_step_ex(0.5, 1.0, 1.0, 1.0, false, 0.2, 0.4, 1.5);
        assert!(fast > slow, "fast={fast} slow={slow}");
        let dry = body_heat_step_ex(0.5, 1.0, 1.0, 1.0, false, 0.03, 0.06, 1.5);
        let wet = body_heat_step_ex(0.5, 1.0, 1.0, 1.0, true, 0.03, 0.06, 3.0);
