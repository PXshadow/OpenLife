...
/// Live-knob variant: `max_boni` / `max_mali` = Haxe Max/MinHealthFoodStoreMaxFactor;
/// `max_age` = Haxe MaxAge.
// Haxe: GlobalPlayerInstance.CalculateHealthFoodStoreMaxFactor
// SETTINGS-LONG-TAIL
#[inline]
pub fn calculate_health_food_store_max_factor_ex(
    yum_multiplier: f32,
    median_prestige: f32,
    true_age: f32,
    max_boni: f32,
    ...
    let max_boni = if max_boni.is_finite() && max_boni > 0.0 {
        max_boni
    } else {
        MAX_HEALTH_FOOD_STORE_MAX_FACTOR
    };
    let max_mali = if max_mali.is_finite() && max_mali > 0.0 {
        max_mali
    } else {
        MIN_HEALTH_FOOD_STORE_MAX_FACTOR
    };
...
/// Live-knob variant: `max_boni` / `max_mali` = Haxe Max/MinHealthAgingFactor;
/// `max_age` = Haxe MaxAge.
// Haxe: GlobalPlayerInstance.CalculateHealthAgeFactor
// SETTINGS-LONG-TAIL
#[inline]
pub fn calculate_health_age_factor_ex(
    yum_multiplier: f32,
    median_prestige: f32,
    true_age: f32,
    max_boni: f32,
...
///
/// `needed_commoner` is Haxe `calculate_needed_prestige` at 0.4 (`ol-identity`).
...
pub fn median_prestige_for_health(needed_commoner_prestige: f32) -> f32 {
    median_prestige_for_health_ex(needed_commoner_prestige, MIN_HEALTH_PER_YEAR)
}

/// Live-knob variant: `min_health_per_year` = Haxe MinHealthPerYear (floor ×30).
// Haxe: GlobalPlayerInstance.calculatePrestigeClass medianPrestige L1078
// SETTINGS-LONG-TAIL
#[inline]
pub fn median_prestige_for_health_ex(
    needed_commoner_prestige: f32,
    min_health_per_year: f32,
) -> f32 {
    ...
    let year = if min_health_per_year.is_finite() && min_health_per_year > 0.0 {
        min_health_per_year
    } else {
        MIN_HEALTH_PER_YEAR
    };
    needed.max(year * 30.0)
}
...
pub fn birth_cross_species_aging_mult_ex(
    ...
    birth_cross_species_aging_mult_live(
        age,
        child_is_human,
        mother_is_ai,
        min_age_to_eat,
...
/// Live `AgingFactorHumanBornToAi` / `AgingFactorAiBornToHuman`.
// Haxe: TimeHelper.updateAge L707-713
pub fn birth_cross_species_aging_mult_live(
    age: f32,
    child_is_human: bool,
    mother_is_ai: Option<bool>,
    min_age_to_eat: f32,
    human_born_to_ai: f32,
    ai_born_to_human: f32,
) -> f32 {
    ...
    match mother_is_ai {
        Some(true) if child_is_human => pick(human_born_to_ai, AGING_FACTOR_HUMAN_BORN_TO_AI),
        Some(false) if !child_is_human => pick(ai_born_to_human, AGING_FACTOR_AI_BORN_TO_HUMAN),
        _ => 1.0,
    }
}
...
pub fn birth_yum_multiplier(account_total_score: f32, median_prestige: f32, true_age: f32) -> f32 {
...
/// Like [`age_step_from_health_ex`] with live starve factor + `GrownUpAge`.
// Haxe: ServerSettings.AgingFactorWhileStarvingToDeath / GrownUpAge (SETTINGS-LONG-TAIL)
pub fn age_step_from_health_live(
    dt: f32,
    age: f32,
    food_store: f32,
    health_age_factor: f32,
    ...
    let starve = if starve_aging_factor.is_finite() && starve_aging_factor > 0.0 {
        starve_aging_factor
    } else {
        AGING_FACTOR_WHILE_STARVING
    };
    let grown_up = if grown_up_age.is_finite() && grown_up_age > 0.0 {
        grown_up_age
    } else {
        GROWN_UP_AGE_YEARS
    };
    ...
    if food < 0.0 {
        if age < grown_up {
            ageing_factor *= starve;
        } else {
            ageing_factor *= 1.0 / starve;
        }
    }
...
/// Haxe `foodDecay *= FoodUseChildFaktor` when `age < GrownUpAge && food_store > 0`.
// Haxe: TimeHelper.updateFoodAndDoHealing L866
// SETTINGS-LONG-TAIL
pub fn child_food_use_mult(age: f32, food: f32, grown_up_age: f32, child_faktor: f32) -> f32 {
    let grown = if grown_up_age.is_finite() && grown_up_age > 0.0 {
        grown_up_age
    } else {
        GROWN_UP_AGE_YEARS
    };
    if age < grown && food > 0.0 {
        if child_faktor.is_finite() && child_faktor >= 0.0 {
            child_faktor
        } else {
...
/// Haxe `foodDecay *= AIFoodUseFactor*` when `player.isAi()` and class is Serf/Commoner/Noble.
///
/// King / Emperor / NotSet stay `1.0` (Haxe if/else; unlike AI speed which maps those to Noble/Commoner).
// Haxe: TimeHelper.updateFoodAndDoHealing L868–872
// SETTINGS-LONG-TAIL
pub fn ai_class_food_use_mult(
    is_ai: bool,
    class: ol_identity::PrestigeClass,
    serf: f32,
    commoner: f32,
    noble: f32,
) -> f32 {
    ...
    match class {
        ol_identity::PrestigeClass::Serf => pick(serf, AI_FOOD_USE_FACTOR_SERF),
        ol_identity::PrestigeClass::Commoner => pick(commoner, AI_FOOD_USE_FACTOR_COMMONER),
        ol_identity::PrestigeClass::Noble => pick(noble, AI_FOOD_USE_FACTOR_NOBLE),
        ol_identity::PrestigeClass::NotSet
        | ol_identity::PrestigeClass::King
        | ol_identity::PrestigeClass::Emperor => 1.0,
    }
}

/// Haxe `isEveOrAdam` — display first name is `EVE` or `ADAM`.
// Haxe: GlobalPlayerInstance.isEveOrAdam L6219–6221
#[inline]
pub fn is_eve_or_adam_name(first_name: &str) -> bool {
    first_name == "EVE" || first_name == "ADAM"
}

/// Haxe `foodDecay *= EveFoodUseFactor` when Eve/Adam and not `isWounded()`.
// Haxe: TimeHelper.updateFoodAndDoHealing L976
// SETTINGS-LONG-TAIL
pub fn eve_food_use_mult(is_eve_or_adam: bool, is_wounded: bool, factor: f32) -> f32 {
    if is_eve_or_adam && !is_wounded {
...
pub struct FoodStoreMaxKnobs {
    ...
    // SETTINGS-LONG-TAIL
    pub starve_reduction: f32,
    /// Haxe `MaxAge` — old-age capacity band start at `max_age - 10`.
    // SETTINGS-LONG-TAIL
    pub max_age: f32,
}
...
impl Default for FoodStoreMaxKnobs {
    fn default() -> Self {
        Self {
            ...
            starve_reduction: FOOD_STORE_MAX_REDUCTION_WHILE_STARVING,
            max_age: FOOD_CAPACITY_MAX_AGE,
        }
    }
}
...
impl FoodStoreMaxKnobs {
    ...
    pub fn sanitize(self) -> Self {
        Self {
            ...
            starve_reduction: if self.starve_reduction.is_finite() && self.starve_reduction >= 0.0 {
                self.starve_reduction
            } else {
                FOOD_STORE_MAX_REDUCTION_WHILE_STARVING
            },
            max_age: if self.max_age.is_finite() && self.max_age > 0.0 {
                self.max_age
            } else {
                FOOD_CAPACITY_MAX_AGE
            },
        }
    }
...
pub fn calculate_food_store_max_ex(input: FoodStoreMaxInput, knobs: FoodStoreMaxKnobs) -> f32 {
    ...
    if food < 0.0 {
        max += knobs.starve_reduction * food;
    }
...
/// Same as [`apply_damage_food_pipe`] with live [`FoodStoreMaxKnobs`]
/// (Haxe `calculateFoodStoreMax` after DoDamage hits/exhaustion).
// Haxe: GlobalPlayerInstance.DoDamage + ServerSettings.*FoodStoreMax
// C-SS-AGE-FOOD-COMBAT
pub fn apply_damage_food_pipe_ex(
    age: f32,
    food_store: f32,
    hits_before: f32,
    exhaustion_before: f32,
...
pub fn can_do_healing(
    ...
    food_store >= 0.0 && !wounded && !has_yellow_fever && angry_time > 0.0 && !super_heat
}
...
pub fn biome_love_factor_for_color(
    ...
    if !mother_or_father && loved <= 0.0 && biome != BIOME_TAG_GREEN && biome != BIOME_TAG_YELLOW {
        loved -= 0.5;
    }
...
pub fn yellow_fever_food_drain(dt: f32) -> f32 {
    yellow_fever_food_drain_ex(dt, EXHAUSTION_YELLOW_FEVER_PER_SEC)
}

/// Live-knob yellow-fever food drain (`ExhaustionYellowFeverPerSec * 2 * dt`).
// Haxe: TimeHelper L447 / ServerSettings.ExhaustionYellowFeverPerSec
// SETTINGS-LONG-TAIL
pub fn yellow_fever_food_drain_ex(dt: f32, exhaustion_yellow_fever_per_sec: f32) -> f32 {
    let dt = if dt.is_finite() && dt > 0.0 { dt } else { 0.0 };
    let rate =
        if exhaustion_yellow_fever_per_sec.is_finite() && exhaustion_yellow_fever_per_sec >= 0.0 {
...
mod tests {
    ...
    fn live_newborn_band_overrides_module() {
        ...
        assert!(
            (food_store_max_from_parts_ex(10.0, 10.0, 0.0, 0.0, 1.0, knobs) - 13.0).abs() < 1e-4
        );
    }
    ...
    fn live_old_age_band_overrides_module() {
        ...
        assert!(
            (food_store_max_from_parts_ex(55.0, 10.0, 0.0, 0.0, 1.0, knobs) - 16.0).abs() < 1e-4
        );
        assert!(
            (food_store_max_from_parts_ex(60.0, 10.0, 0.0, 0.0, 1.0, knobs) - 12.0).abs() < 1e-4
        );
    }
    ...
    fn live_combined_grown_newborn_old() {
        ...
        assert!(
            (food_store_max_from_parts_ex(10.0, 10.0, 0.0, 0.0, 1.0, knobs) - 15.0).abs() < 1e-4
        );
        // adult ignores newborn/old
        assert!(
            (food_store_max_from_parts_ex(30.0, 10.0, 0.0, 0.0, 1.0, knobs) - 25.0).abs() < 1e-4
        );
        // old mid: 8 + 5/10*(25-8) = 16.5
        assert!(
            (food_store_max_from_parts_ex(55.0, 10.0, 0.0, 0.0, 1.0, knobs) - 16.5).abs() < 1e-4
        );
    }
    ...
    fn adult_ignores_live_newborn_and_old_age() {
        ...
        assert!(
            (food_store_max_from_parts_ex(30.0, 10.0, 0.0, 0.0, 1.0, knobs) - 20.0).abs() < 1e-4
        );
        assert!(
            (food_store_max_from_parts_ex(50.0, 10.0, 0.0, 0.0, 1.0, knobs) - 20.0).abs() < 1e-4
        );
    }

    #[test]
    fn live_max_age_old_band_starts_earlier() {
        // default MaxAge 60: age 45 is adult (old band starts at 50)
        assert!(
            (food_store_max_from_parts_ex(45.0, 10.0, 0.0, 0.0, 1.0, FoodStoreMaxKnobs::default())
                - 20.0)
                .abs()
                < 1e-4
        );
        // live MaxAge 50: old band starts at 40; age 45 → 10 + 5/10*(20-10) = 15
        let knobs = FoodStoreMaxKnobs {
            max_age: 50.0,
            ..FoodStoreMaxKnobs::default()
        };
        assert!(
            (food_store_max_from_parts_ex(45.0, 10.0, 0.0, 0.0, 1.0, knobs) - 15.0).abs() < 1e-4
        );
    }
    ...
    fn live_starve_reduction_overrides_module() {
        ...
        let knobs = FoodStoreMaxKnobs {
            starve_reduction: 3.0,
            ..FoodStoreMaxKnobs::default()
        };
        let m = food_store_max_from_parts_ex(30.0, -2.0, 0.0, 0.0, 1.0, knobs);
        assert!((m - 14.0).abs() < 1e-4);
    }
    ...
    /// C-SS-AGE-FOOD-COMBAT: DoDamage recompute uses live NewBorn / OldAge bands.
    // Haxe: ServerSettings.NewBornFoodStoreMax / OldAgeFoodStoreMax
    #[test]
    fn apply_damage_food_pipe_ex_live_age_bands() {
        let knobs = FoodStoreMaxKnobs {
            newborn: 8.0,
            old_age: 14.0,
            ..FoodStoreMaxKnobs::default()
        };
        let baby = apply_damage_food_pipe_ex(0.0, 4.0, 0.0, 0.0, 2.0, 1.0, true, knobs);
        let baby_default = apply_damage_food_pipe(0.0, 4.0, 0.0, 0.0, 2.0, 1.0, true);
        // live 8 − 2 hits − 2 exh (floor 3) vs default 4 − 2 − 2 (floor 1)
        assert!((baby.food_store_max - 4.0).abs() < 1e-4);
        assert!((baby_default.food_store_max - 1.0).abs() < 1e-4);
        let elder = apply_damage_food_pipe_ex(60.0, 10.0, 0.0, 0.0, 2.0, 1.0, true, knobs);
        let elder_default = apply_damage_food_pipe(60.0, 10.0, 0.0, 0.0, 2.0, 1.0, true);
        // live 14 − 2 − 2 vs default 10 − 2 − 2
        assert!((elder.food_store_max - 10.0).abs() < 1e-4);
        assert!((elder_default.food_store_max - 6.0).abs() < 1e-4);
    }

    #[test]
    fn heal_exhaustion_increases_drain() {
        let step = step_healing_food_pipe(30.0, 10.0, 0.0, 5.0, 1.0, 1.0, 0.10, true, false);
    ...
    fn heal_exhaustion_live_factor_override() {
        let base = step_healing_food_pipe(30.0, 10.0, 0.0, 5.0, 1.0, 1.0, 0.10, true, false);
        ...
        let female = step_healing_food_pipe(30.0, 10.0, 0.0, 5.0, 1.0, 1.0, 0.10, true, false);
    ...
    fn heal_hits_restores_max() {
        ...
        let step = step_healing_food_pipe(30.0, 10.0, 4.0, -20.0, 1.0, 1.0, 0.10, true, false);
    ...
    fn heal_hits_live_wound_healing_factor_override() {
        let base = step_healing_food_pipe(30.0, 10.0, 4.0, -20.0, 1.0, 1.0, 0.10, true, false);
        let double = step_healing_food_pipe_ex(
            30.0,
            10.0,
            4.0,
    ...
    fn biome_love_factor_brown_jungle_and_swamp_floor() {
        ...
        let love = biome_love_factor(BIOME_TAG_JUNGLE, 0, brown, Some(brown), Some(brown));
        ...
        assert!((biome_love_factor(BIOME_TAG_SWAMP, 100, brown, None, None) - (-3.0)).abs() < 1e-6);
    ...
    fn step_healing_applies_temp_damage_and_biome_love() {
        ...
        let expect_exh =
            5.0 + 0.10 * TEMPERATURE_EXHAUSTION_DAMAGE_FACTOR - HEALING_PER_SECOND * 0.5 * 1.0;
    ...
    fn yellow_fever_food_and_heat_vitals() {
        ...
        assert!((yellow_fever_food_drain_ex(1.0, 0.25) - 0.5).abs() < 1e-6);
    ...
    fn median_prestige_floors_at_thirty() {
        ...
        // live MinHealthPerYear 2 → floor 60
        assert!((median_prestige_for_health_ex(0.0, 2.0) - 60.0).abs() < 1e-5);
        assert!((median_prestige_for_health_ex(70.0, 2.0) - 70.0).abs() < 1e-5);
    }
    ...
    fn health_food_max_factor_high_yum_above_one() {
        ...
        // compiled Max/MinHealthFoodStoreMaxFactor 1.2 / 0.8
        assert!((MAX_HEALTH_FOOD_STORE_MAX_FACTOR - 1.2).abs() < 1e-6);
        assert!((MIN_HEALTH_FOOD_STORE_MAX_FACTOR - 0.8).abs() < 1e-6);
        // live MaxHealthFoodStoreMaxFactor 1.5 → (1.5*60+30)/90 = 1.333…
        let live = calculate_health_food_store_max_factor_ex(
            60.0,
            30.0,
            0.0,
            1.5,
        ...
        assert!((live - 1.333333).abs() < 1e-3);
        // live MinHealthFoodStoreMaxFactor 0.5 mali: health=-30 → (-60)/((1/0.5)*(-30)-30)=0.666…
        let mali = calculate_health_food_store_max_factor_ex(
            0.0,
            30.0,
            30.0,
            1.2,
        ...
        assert!((mali - 0.666666).abs() < 1e-3);
        let compiled_mali = calculate_health_food_store_max_factor(0.0, 30.0, 30.0);
        // (-60)/((1/0.8)*(-30)-30)=0.888…
        assert!((compiled_mali - 0.888888).abs() < 1e-3);
    }
    ...
    fn health_age_factor_high_yum_above_one() {
        ...
        // live MaxHealthAgingFactor 3 → (3*60+30)/90 = 2.333…
        let live =
            calculate_health_age_factor_ex(60.0, 30.0, 0.0, 3.0, 0.5, HEALTH_MEDIAN_MAX_AGE);
        assert!((live - 2.333333).abs() < 1e-3);
    }
    ...
    fn health_age_factor_low_yum_below_one() {
        ...
        // live MinHealthAgingFactor 0.25 → (-60)/((1/0.25)*(-30)-30)=0.4
        let mali =
            calculate_health_age_factor_ex(0.0, 30.0, 30.0, 2.0, 0.25, HEALTH_MEDIAN_MAX_AGE);
        assert!((mali - 0.4).abs() < 1e-3);
        // live MaxAge 30: trueAge 30 → health -60 → mali 0.5 → 0.6
        let age = calculate_health_age_factor_ex(0.0, 30.0, 30.0, 2.0, 0.5, 30.0);
        assert!((age - 0.6).abs() < 1e-3);
    }

    #[test]
    fn live_max_age_old_age_band() {
        // Haxe MaxAge 50: old band starts > 40; age 45 → 10 + 5/10 * 10 = 15
        let knobs = FoodStoreMaxKnobs {
            max_age: 50.0,
            ..FoodStoreMaxKnobs::default()
        };
        ...
        // compiled MaxAge 60: age 45 still adult 20
        assert!((food_store_max_from_parts(45.0, 10.0, 0.0, 0.0, 1.0) - 20.0).abs() < 1e-4);
    }
    ...
    fn birth_mult_human_infant_ai_mother() {
        ...
        assert!(
            (birth_cross_species_aging_mult(1.0, true, Some(true)) - AGING_FACTOR_HUMAN_BORN_TO_AI)
                .abs()
                < 1e-6
        );
        // age >= 3 → 1
    ...
    #[test]
    fn age_step_live_starve_factor_override() {
        // Youth starve ×0.25 instead of 0.5
        let s = age_step_from_health_live(60.0, 10.0, -1.0, 1.0, 1.0, 60.0, 0.25, 14.0);
        assert!((s.ageing_factor - 0.25).abs() < 1e-5);
        assert!((s.age_delta - 0.25).abs() < 1e-5);
        assert!((s.true_age_delta - 1.0).abs() < 1e-5);
        // Adult starve ×1/0.25 = 4
        let a = age_step_from_health_live(60.0, 20.0, -1.0, 1.0, 1.0, 60.0, 0.25, 14.0);
        assert!((a.ageing_factor - 4.0).abs() < 1e-5);
        assert!((a.age_delta - 4.0).abs() < 1e-5);
    }

    #[test]
    fn age_step_live_grown_up_age_override() {
        // Age 20 is adult at default 14 (hf=2 → ageingFactor 0.5); youth if GrownUpAge=25.
        let adult = age_step_from_health_live(60.0, 20.0, 10.0, 2.0, 1.0, 60.0, 0.5, 14.0);
        assert!((adult.ageing_factor - 0.5).abs() < 1e-5);
        let youth = age_step_from_health_live(60.0, 20.0, 10.0, 2.0, 1.0, 60.0, 0.5, 25.0);
        assert!((youth.ageing_factor - 1.0).abs() < 1e-5);
        // Starve: 20 is youth at GrownUpAge=25 → ×0.5 not ×2
        let starve_youth = age_step_from_health_live(60.0, 20.0, -1.0, 1.0, 1.0, 60.0, 0.5, 25.0);
        assert!((starve_youth.ageing_factor - 0.5).abs() < 1e-5);
    }

    #[test]
    fn child_food_use_mult_haxe_gate() {
        assert!((child_food_use_mult(10.0, 5.0, 14.0, 1.0) - 1.0).abs() < 1e-6);
        assert!((child_food_use_mult(10.0, 5.0, 14.0, 2.0) - 2.0).abs() < 1e-6);
        // adult
        assert!((child_food_use_mult(20.0, 5.0, 14.0, 2.0) - 1.0).abs() < 1e-6);
        // child but food not > 0
        assert!((child_food_use_mult(10.0, 0.0, 14.0, 2.0) - 1.0).abs() < 1e-6);
        assert!((child_food_use_mult(10.0, -1.0, 14.0, 2.0) - 1.0).abs() < 1e-6);
    }

    /// Haxe AI class foodDecay: humans 1.0; King/Emperor/NotSet 1.0; Serf/Commoner/Noble live knobs.
    // Haxe: TimeHelper.updateFoodAndDoHealing L868–872
    #[test]
    fn ai_class_food_use_mult_haxe_gate() {
        use ol_identity::PrestigeClass;
        assert!(
            (ai_class_food_use_mult(false, PrestigeClass::Serf, 0.5, 0.6, 0.7) - 1.0).abs() < 1e-6
        );
        assert!(
            (ai_class_food_use_mult(true, PrestigeClass::Serf, 0.8, 0.9, 1.0) - 0.8).abs() < 1e-6
        );
        assert!(
            (ai_class_food_use_mult(true, PrestigeClass::Commoner, 0.8, 0.9, 1.0) - 0.9).abs()
                < 1e-6
        );
        assert!(
            (ai_class_food_use_mult(true, PrestigeClass::Noble, 0.8, 0.9, 1.0) - 1.0).abs() < 1e-6
        );
        assert!(
            (ai_class_food_use_mult(true, PrestigeClass::King, 0.8, 0.9, 1.0) - 1.0).abs() < 1e-6
        );
        assert!(
            (ai_class_food_use_mult(true, PrestigeClass::Emperor, 0.5, 0.5, 0.5) - 1.0).abs()
                < 1e-6
        );
        assert!(
            (ai_class_food_use_mult(true, PrestigeClass::NotSet, 0.5, 0.5, 0.5) - 1.0).abs() < 1e-6
        );
        assert!(
            (ai_class_food_use_mult(true, PrestigeClass::Serf, 0.4, 0.9, 1.0) - 0.4).abs() < 1e-6
        );
        assert!(
            (ai_class_food_use_mult(true, PrestigeClass::Serf, f32::NAN, 0.9, 1.0)
    ...
    /// Haxe EveFoodUseFactor: unwounded EVE/ADAM only.
    // Haxe: TimeHelper.updateFoodAndDoHealing L976
    #[test]
    fn eve_food_use_mult_haxe_gate() {
        assert!(is_eve_or_adam_name("EVE"));
        assert!(is_eve_or_adam_name("ADAM"));
        assert!(!is_eve_or_adam_name("eve"));
        assert!(!is_eve_or_adam_name("LISA"));
        assert!((eve_food_use_mult(true, false, 0.5) - 0.5).abs() < 1e-6);
        assert!((eve_food_use_mult(true, true, 0.5) - 1.0).abs() < 1e-6);
        assert!((eve_food_use_mult(false, false, 0.5) - 1.0).abs() < 1e-6);
        assert!((eve_food_use_mult(true, false, 1.0) - 1.0).abs() < 1e-6);
        assert!((eve_food_use_mult(true, false, f32::NAN) - EVE_FOOD_USE_FACTOR).abs() < 1e-6);
    }
    ...
    fn birth_floor_cancels_age_health_penalty() {
        ...
        assert!(
            (f - 1.0).abs() < 1e-3,
            "neutral health at birth floor, got {f}"
        );
    }
    ...
    #[test]
    fn birth_cross_species_aging_mult_live_factors() {
        assert!(
            (birth_cross_species_aging_mult_live(1.0, true, Some(true), 3.0, 5.0, 2.0) - 5.0).abs()
                < 1e-6
        );
        assert!(
            (birth_cross_species_aging_mult_live(1.0, false, Some(false), 3.0, 5.0, 2.0) - 2.0)
                .abs()
                < 1e-6
        );
    }
