...
| L-HUD | hunger boxes, arrows, slips, fonts | **DONE→stronger** | OLHU v2 fonts; slips; yum/last-ate names; **craving sheet line**; curse tokens; home arrows. Out of scope: life-token count, tool-slots HUD |
...
| L-RENDER | software scene (ysort, ground, clothing, containers, screen_to_world) | **DONE→stronger** (`render.rs`: PE mid-arm + eyes/head before hat; frozenArm t=0; leadership badge; apocalypse overlay; rideable split; wall sub-order). Non-goal: wgpu world author |
